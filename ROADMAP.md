...
The formal check stays with the next matrix pass, which is also where
items 2 through 5 of the T29 queue belong.
...
2507). The five below survive. Items 2 through 5 are all about the EVAL
HARNESS rather than the product, and each one changes what a published
number means, so they belong to a milestone that re-runs the matrix and
can restate the scores in the same breath; item 9 needs a live model to
reproduce at all.
