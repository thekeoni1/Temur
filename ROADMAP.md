...
- **The prose-call recovery path deserves its own measurement.** T37
  showed temur's entire score on Qwen2.5-Coder-3B resting on it, which
  no test asserts directly. A recovery-disabled run against the same
  model would turn "the harness is the difference" from an inference
  into a measured control, and it is cheap: one env knob and one cell.
