...
| T15 | Model-selection onboarding polish (PLANNED) | Close the four gaps between "easy for Ollama veterans" and "easy for everyone" found in the 2026-07-28 UX walkthrough: `temur init` probes the endpoint's `/v1/models` and offers a picker instead of the free-text model-id prompt, falling back to free text when unreachable (P1); a way to persist a raw-id `/model` switch to config instead of session-only (P2); `temur doctor` compares the configured model against the server's `/v1/models` and flags a model the server doesn't have - the most likely new-user misconfig (P3); surface the OFFLINE.md shortlist/model guidance to binary-install users who never see the repo docs (P4). All four fit existing patterns (init already prompts interactively, doctor already probes endpoints, commands already hold the resolved config); no architectural change |
